...
function getallAppointments(){
    ...
    var id = patient.id;
    xhr.open('POST', baseUrl+"/doctor/getvisitedappointments", true); 
        ...
        if(xhr.status === 200 && xhr.readyState === 4){
            ...
            if(response.data === "none"){
                ...
                for(var i in response.data){
                    var allappointments = '	<tr><td><h2 class="table-avatar">'+ response.data[i].fullname +'</h2></td><td>'+response.data[i].email +'</td><td>'+response.data[i].idnumber+'</td><td>'+response.data[i].appointdate+'</td><td>$'+response.data[i].amount+'</td><td>'+response.data[i].doccomment+'</td><td>'+response.data[i].status+'</td></tr>'
