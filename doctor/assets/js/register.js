...
function auth(e) {
  var xhr = new XMLHttpRequest();
    ...
    var proffession = $('#regproffession').val();
    xhr.open('POST', baseUrl+"/doctor/register", true); 
    ...
    xhr.onreadystatechange = function(){
        ...
        if(xhr.status === 200 && xhr.readyState === 4){
            ...
            if(response.data === "registered"){
              window.location = "http://localhost:8080/appointmentsystem/doctor/login.html";
            }
            else{
    ...
    xhr.send('email='+email+'&password='+password+'&fullname='+fullname+'&phone='+phone+'&expertin='+proffession);  
