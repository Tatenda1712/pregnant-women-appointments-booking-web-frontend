...
function getallAppointments(){
    ...
    var id = patient.id;
    xhr.open('POST', baseUrl+"/admin/doctors", true); 
        ...
        if(xhr.status === 200 && xhr.readyState === 4){
            ...
            if(response.data === "none"){
                ...
                for(var i in response.data){
                    var allappointments = '	<tr><td>'+response.data[i].fullname+'</td><td>'+response.data[i].email+'</td><td>'+response.data[i].phone+'</td><td>'+response.data[i].expertin+'</td></tr>'
